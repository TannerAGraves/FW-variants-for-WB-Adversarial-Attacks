...
%constraint set
The $L_p$ constraint $||x||_p \leq \epsilon$ directly restricts the size of perturbations made the to the example. An inherient problem of DNNs is often attacks can be consistently sucessful even with very small, even imperceptable $\epsilon$. Different choices of $p$ may be made giving $||x||_p = (\sum_i{x_i^p})^{1/p}$. We have implemented and provide results for the $L_2$ and the $L_\infty(x) = \max_i |x_i|$ norms, focusing mostly on the ladder. 
...
We implement attacks Linear Minimization orcales to enable both $L_\infty$ and $L_2$ constrained attacks. 
Choices of $\epsilon$ for the two norms are clearly not comparable as $L_\infty$ is conserned by the maximum pertrubation a pixel is perturbed by, doing nothing to disencentivise most if not all pixels from being pertrubed, where $L_2$ constrains the sum of squares of pertrubations. 
...
To compare the resulting adversarial examples generated by attacks I chose $\epsilon$ values that gave approximately the same attack success rate of aboout $62\%$ on CIFAR-10 ($\epsilon_\infty = 0.005, \epsilon_2 = 0.02$) and compare the $L1$ norm of the generated perturbations. For the to get the same success rate we expectedly see that $L2$ constrained attacks are more efficient at producting attacks, with an average $L1$ of $6.84$ ($\sigma = 0.69$) where $L_\infty$ constrained attacks created attacks with average perturbation $L_1$ of $10.5$. However, we note that both cases produced attacks below with marginal perceptibility with $L_2$ attacks having an average $L_\infty$ of $0.013$ and it being difficult to think of applicaitons were this efficiency is worth the trade off. 
...
We found the application of linesearching techniques to be marginally benificial when compared to decaying rule.
